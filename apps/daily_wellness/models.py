...
class WellnessTip(models.Model):
    ...
    Safety exclusion tags are checked in Python before a tip is ever shown
    or sent to an LLM -- this is what guarantees an allergy or condition
    can never be ignored, regardless of what any AI-generated wording does.
    """
...
class DailyWellnessLog(models.Model):
    """One generated-and-cached plan per mother per calendar day."""

    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="daily_wellness_logs")
    date = models.DateField()

    nutrition_tip = models.ForeignKey(WellnessTip, null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
    mental_health_tip = models.ForeignKey(WellnessTip, null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
    exercise_tip = models.ForeignKey(WellnessTip, null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
    precaution_tip = models.ForeignKey(WellnessTip, null=True, blank=True, on_delete=models.SET_NULL, related_name="+")
